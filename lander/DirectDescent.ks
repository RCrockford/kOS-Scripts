...
if Ship:Status = "Flying" or Ship:Status = "Sub_Orbital" or Ship:Status = "Escaping"
{
	...
	if Stage:Number > 0
	{
		...
		if Ship:Body:Mu / LAS_ShipPos():SqrMagnitude < 0.5
		{
			print "Waiting for initial gravity to increase".
			wait until Ship:Body:Mu / LAS_ShipPos():SqrMagnitude >= 0.5.
		}
		...
		wait until Alt:Radar < d + Ship:Velocity:Surface:Mag * 80.
		...
		wait until Ship:Velocity:Surface:Mag < 50 or vdot(Facing:Vector, SrfRetrograde:Vector) < 0.8.	// stop burn if too far from retrograde
    ...
    local hFactor is 1 + 3.4 / shipBounds:BottomAltRadar^(0.7 / sqrt(twr)).

    print "TWR=" + round(twr, 2) + " hF=" + round(hFactor, 3).
    
    local debugGui is GUI(400, 80).
    set debugGui:X to -180.
    ...
    until shipBounds:BottomAltRadar < 0.1
    {
        ...
        // A little hysteresis
        if fr > (0.9 - Ship:Control:PilotMainThrottle * 0.05)
