// Raise periapsis using RCS or engines
...
// Calc required dV
local currentV is sqrt(Ship:Body:Mu * Ship:Orbit:SemiMajorAxis * (1 - Ship:Orbit:Eccentricity^2)) / (Ship:Orbit:Apoapsis + Ship:Body:Radius).

set targetPe to targetPe * 1000 + Ship:Body:Radius.

local targetA is (Ship:Orbit:Apoapsis + Ship:Body:Radius + targetPe) / 2.
local targetEcc is 1 - targetPe / targetA.
local targetV is sqrt(Ship:Body:Mu * targetA * (1 - targetEcc^2)) / (Ship:Orbit:Apoapsis + Ship:Body:Radius).
...
print "Executing manoeuvre at Ap-" + round(duration * 0.5, 1) + " seconds.".
print "  DeltaV: " + round(deltaV, 1) + " m/s.".
print "  Duration: " + round(duration, 1) + " s.".

local burnEta is Eta:Apoapsis - duration * 0.5.
...
if useEngines
    runpath("0:/flight/SetupBurn", burnParams, list("flight/RaisePeriBurn.ks", "FCFuncs.ks" "flight/EngineMgmt.ks")).
else
    runpath("0:/flight/SetupBurn", burnParams, list("flight/RaisePeriRCS.ks")).
